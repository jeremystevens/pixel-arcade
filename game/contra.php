                ...
                <div class="retro-box bg-dark-purple border-2 border-neon-purple p-6 rounded-lg mb-6">
                    <div class="aspect-square bg-gray-800 rounded-lg mb-4 overflow-hidden">
                        ...
                        foreach ($possibleFormats as $format) {
                            if (file_exists(__DIR__ . "/../assets/images/games/{$gameSlug}/box-art.{$format}")) {
                                $boxArtPath = "/assets/images/games/{$gameSlug}/box-art.{$format}";
                                break;
                            }
                        }
                        ?>
                        ...
                        <?php else: ?>
                            <div class="w-full h-full flex items-center justify-center">
                                <i class="fas fa-gamepad text-8xl text-neon-purple"></i>
                            </div>
                        <?php endif; ?>
                    </div>
                ...
                <div class="retro-box bg-dark-purple border-2 border-neon-purple p-6 rounded-lg">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <?php for ($i = 1; $i <= 4; $i++): ?>
                            ...
                            $possibleFormats = ['jpg', 'png', 'svg', 'webp'];
                            foreach ($possibleFormats as $format) {
                                if (file_exists(__DIR__ . "/../assets/images/games/{$gameSlug}/screenshot-{$i}.{$format}")) {
                                    $screenshotPath = "/assets/images/games/{$gameSlug}/screenshot-{$i}.{$format}";
                                    break;
                                }
                            }
                            ?>
                            <?php if ($screenshotPath): ?>
                                <img src="<?php echo htmlspecialchars($screenshotPath); ?>" 
                ...
                <?php if (empty($scores)): ?>
                    <div class="retro-box bg-dark-purple border-2 border-neon-purple p-6 rounded-lg">
                        ...
                        <div class="text-center py-12">
                            ...
                            <p class="text-sm text-gray-400 mt-2">Be the first to set a high score!</p>
                        </div>
                    </div>
                <?php else: ?>
                    <?php 
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            if (typeof MIDIPlayer !== 'undefined') {
                const player = new MIDIPlayer('/assets/music/<?php echo $gameSlug; ?>.mid');
                player.init();
            }
        });
    </script>
